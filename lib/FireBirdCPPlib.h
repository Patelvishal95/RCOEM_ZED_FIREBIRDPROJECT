...
public:
		...
		BOOL setPosition(void *hSerial,uint32 leftposition, signed short int leftVelocity,uint32 rightposition,signed short int rightVelocity);
		...
		BOOL getEncoderticks_mm(void *hSerial, uint16 *ticks_mm);
		BOOL getEncoderTicksperRevolution(void *hSerial, uint16 *ticks_mm);
};
